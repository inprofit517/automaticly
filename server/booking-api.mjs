// Automaticly – Erstgespräch booking API
...
//
// Run with:  npm run server   (which loads .env via node --env-file)
//
// If MS_CLIENT_SECRET is empty/placeholder, the API runs in MOCK mode:
// it returns synthetic availability and fake-confirms bookings so the UI is
...
const MOCK = PLACEHOLDER.test(CFG.clientSecret) || !CFG.tenantId || !CFG.clientId;
...
// Booking
// ---------------------------------------------------------------------------
async function createBooking({ start, name, email, phone, message }) {
